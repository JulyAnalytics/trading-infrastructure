...
| `GET /api/sarah/compare?tickers=` | **batch compare** — latest vol_signals row per requested ticker (comma/space/newline separated, capped 30) + derived `iv_rv_spread` + `rankings` (tickers sorted desc per numeric field: iv_rank, vrp_proxy_bkwd, skew_25d_rr, atm_iv_30d, ts_front_slope, rv_21d). Pure DB read; 404 if none |
| `GET /api/sarah/charts/term-structure-compare?tickers=` | multi-ticker ATM-IV-by-DTE overlay figure (one trace per ticker, parsed from each row's `term_structure_json`) |
| `GET /api/sarah/charts/iv-history-compare?tickers=&days=126` | multi-ticker trailing ATM IV 30d overlay figure |
| `GET /api/sarah/charts/scatter-vol-map?tickers=` | vol opportunity scatter: x=iv_rank, y=VRP spread (IV−RV), size∝ATM IV, quadrant lines at 50/0 |
